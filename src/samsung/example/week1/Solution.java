...
class Solution {
    ...
    private static boolean isValid(int guess[]) {
        ...
        for (int idx = 0; idx < N; ++idx) {
            if (guess[idx] < 0 || guess[idx] >= 10 || guess_c[guess[idx]] > 0) return false; // 1~9 사이의 숫자만
            guess_c[guess[idx]]++;
        }
        return true;
    }
    ...
    private static void initialize(Scanner sc) {
        for (int count = 0; count < 10; ++count) digits_c[count] = 0; // digits_c 초기화

        String input;   // 시스템 비밀번호 입력
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        System.setIn(new java.io.FileInputStream("C:\\Users\\ChoiJunho\\Desktop\\sample\\sample_input.txt"));
        ...
        for (int testcase = 1; testcase <= T; ++testcase) {
            ...
            int guess[] = new int[N];
            usersolution.doUserImplementation(guess); // guess 배열 인자로 doUserImplementation 호출
